...
- [ ] 2.1 `npm run typecheck` passes with updated `StylePicker.tsx`
- [ ] 2.2 `npm run build` completes without errors

#### Manual

- [ ] 2.3 Library tab renders public styles for the object's category on first click
- [ ] 2.4 Selecting a library style and clicking Transform triggers POST `/start` with UUID `style_name`
- [ ] 2.5 Empty library state shows "No library styles" message with `/styles/new` link
- [ ] 2.6 Switching back to Presets tab works without re-fetching
- [ ] 2.7 Mobile 375px: tabs and cards accessible
